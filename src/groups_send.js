	...
	async sendMsg( member, msg, hongbao ) {
		...
		if ( msg.msgtype == 1 ) {
			...
			for (var i = 0; i < rooms.length; i++) {
				...
				//非群主时，移除 @all 标记
				if( msg.source && msg.source.indexOf('notify@all') > -1 && ( rooms[i].master !== 1 && rooms[i].master != member.weixin_id ) ){
					msg.source = '';
		...
		for (var i = 0; i < rooms.length; i++) {
			...
			if (msg.msgtype == 90) {
				...
				var fn = this.wx.instance( member.auto_id, member.device_id ).SendAppMsgXml(member.weixin_id, chat, body);
