...
class GroupsSend {
    ...
    getMember(sendroomid, func) {
        ...
        //用户列表最近 5 分钟内更新过
        if (self.members[sendroomid] != undefined && self.members[sendroomid].length && com.getTime() - self.updated <= 60 * 5) {
            return func();
        }
        ...
        self.mysql.query(sql, [date, time, 1204583], function (err, res) {
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                if (roomid.length > 0) {
                    ...
                    member.push({ member_id: res[i].member_id, weixin_id: res[i].weixin_id, tag: res[i].tag, roomid, roomidInfo });
                }
            ...
            self.members[sendroomid] = member;

            self.updated = com.getTime();

            act.record(self.mysql, self.item, { 'quantity': useids.length, 'member_ids': useids }, '筛选用户');

            log.info('筛选用户', '在线用户 ' + res.length + ' 人，群发用户（' + sendroomid + '）' + member.length + ' 人，发送状态 ' + self.sender);
