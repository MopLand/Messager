...
class Weixin {
	...
	request(method, api, arg, valid = null ) {
		...
		*/
		...
		var url = host + api + ( api.indexOf('?') > -1 ? '&' : '?' ) + 'instance=' + self.inst + '&deviceId=' + self.devi;
		...
		//if( method == 'post' ){
		//	arg['deviceId'] = self.devi;
		//}else{
		//	url += '&deviceId=' + self.devi;
		//}
		...
		return new Promise(function (resolve, reject) {
			...
			Request[method](url, arg, ( code, body ) => {
				...
				if ( data = valid( code, body ) ) {
					...
					self.code = body?.baseResponse?.ret || code;
					...
					reject( err );
					log.error( method, { 'request' : api, 'device' : self.devi, 'querys' : arg, 'instance' : self.inst, 'response' : body, 'server' : host, 'error' : err } );
	...
	UploadVideoXml(userName, toUserName, content, msgSource = '' ) {
		var api = 'message/sendvideo?' + Common.buildReq( { userName } );
		var ret = { toUserName, content, msgSource };
		return this.post(api, ret);
	}
