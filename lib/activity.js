...
class Activity {
	...
	*/
     ...
     * @param object 扩展信息
     */
	static collect( caller, method, product, rawdata, extra = {} ){
		...
		*/
		...
		//调用存储过程写入数据
		//CALL func_weixin_effect(`package`, `method`, `platform`, `item_id`, `extra`, `created_time`)
		
		let sql = 'CALL func_weixin_effect( ?, ?, ?, ?, ?, ? )';
		let req = [ rawdata.package, method, product.platform, product.item_id, JSON.stringify(extra), rawdata.created ];
