...
class Activity {
	...
	*/
	static detectTbc(text, size) {
		...
		return new RegExp('(\\W+)([a-zA-Z0-9]{'+ ( size || '10,14' ) +'})(\\W+)').exec( text );
	}
	...
	*/
     ...
     * @param object 扩展信息
     */
	static collect( db, method, product, rawdata, extra = {} ){
		...
		if( this.queue[ hash ] ){
			...
			//追加群信息
			if( method == 'groups' ){
				extra = Object.assign( this.goods[ hash ].extra, extra );
			}else{
				extra = this.goods[ hash ].extra;
			}

			let sql = 'UPDATE `pre_weixin_effect` SET sends_num = sends_num + 1, updated_time = ?, extend = ? WHERE method = ? AND platform = ? AND item_suffix = ? AND created_date = ?';
			let req = [ time, JSON.stringify( extra ), method, product.platform, tail, yymd ];

			db.query(sql, req);
			
		}else{

			this.queue[ hash ] = { product, extra };
			...
			let req = [ rawdata.package, method, product.platform, rwid, tail, 1, JSON.stringify( extra ), rawdata.created, yymd ];

			db.query(sql, req);

			/*
			let sql = 'SELECT `auto_id`, `platform`, `method`, `item_id`, `created_time` FROM `pre_weixin_effect` WHERE method = ? AND platform = ? AND item_suffix = ? AND created_date = ? AND package = ? ORDER BY `created_time` DESC LIMIT 1';
			let req = [ method, product.platform, tail, date, msgid ];
			...
			db.query(sql, req, ( err, res ) => {
				...
				if ( res.length > 0 ) {
					let usql = 'UPDATE `pre_weixin_effect` SET sends_num = sends_num + 1, updated_time = ? WHERE method = ? AND platform = ? AND item_suffix = ? AND created_date = ? AND package = ?';
