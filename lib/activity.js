        ...
        memo.get( hash, (err, ret) => {
			...
			if( ret ){
				...
				if( obj.id ){
					...
					//追加群信息
					let ext = ( method == 'groups' ? Object.assign( obj.ext, extra ) : ret );
					...
					caller.mysql.query(sql, req);

				}else{

					//追加群信息
					let ext = ( method == 'groups' ? JSON.stringify( Object.assign( obj, extra ) ) : raw );
					...
					let req = [ time, ext, method, product.platform, tail, yymd ];
