	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+\.)?(3|u\.f(\d)url|jd|vip|tmall|taobao|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|tb|mt|meituan|dpurl|didi|suvmothq|yuebai|yuebuy|maishou|lg(\d+)|kzurl(\d+))\.(com|cc|hk|cn|tv|top|love|plus|tv|fit|art|pub|ren|link)/i.test( text );
		}
	}
