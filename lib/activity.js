...
class Activity {
	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+\.)?(3|u\.f(\d)url|jd|vip|tmall|taobao|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|tb|meituan|dpurl|didi|suvmothq|yuebai|yuebuy|maishou)\.(com|hk|cn|tv|plus|top|love|ren|link)/i.test( text );
		}
	}
	...
	*/
	static detectApp(text) {
		return /#小程序:\/\/(快手|拼多多)(特惠|福利券|限时折扣|短视频|优惠商品推荐)\/(.+)/i.test( text ) || /mp:\/\/(.+)/i.test( text );
	}
