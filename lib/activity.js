...
class Activity {
	...
	*/
	static detectTbc(text, size) {
		...
		return new RegExp('(\\W+)((?=.*[a-zA-Z])[a-zA-Z0-9]{'+ ( size || '10,14' ) +'})(\\W+)').exec( text );
	}
	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+)\.(jd|vip|tmall|taobao|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|tb)\.(com|hk|cn)/i.test( text );
		}
	}
	...
	static collect( caller, method, product, rawdata, extra = {} ){
        ...
        memo.get( hash, (err, ret) => {
			...
			if( ret ){
				...
				let ext = ( typeof obj.ext == 'object' ? Object.assign( obj.ext, extra ) : ret );
