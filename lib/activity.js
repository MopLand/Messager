	...
	static collect( caller, method, product, rawdata, extra = {} ){
        ...
        caller.redis.get( hash, (err, ret) => {
			...
			if( ret ){
				...
				//追加群信息
				let ext = ( method == 'groups' ? JSON.stringify( Object.assign( JSON.parse(ret), extra ) ) : ret );
				...
				//临时存储一天
				caller.redis.set(hash, ext);
				caller.redis.expire(hash, 3600);
