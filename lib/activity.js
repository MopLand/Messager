...
class Activity {
	...
	*/
	...
	static collect( db, method, product ){
		...
		//商品清单
		if( typeof this.goods == 'undefined' ){
			this.goods = {};
		}
		...
		if( this.goods[ product.item_id ] ){
			db.query('UPDATE `pre_order_effect` SET sends_num = sends_num + 1, updated_time = ? WHERE item_id = ? AND created_date = ?', [ time, product.item_id, date ] );			
		}else{
