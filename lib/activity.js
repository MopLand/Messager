	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+\.)?(3|u\.f(\d)url|jd|vip|tmall|taobao|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|tb|mt|meituan|dpurl|didi|huaxz|suvmothq|yuebai|yuebuy|maishou|my22|lg22|mlj94|xingfan|navo|5kma|lg(\d+)|kurl(\d+)|kzurl(\d+))\.(com|cc|co|hk|cn|tv|fun|info|top|love|plus|tv|fit|art|pub|red|ren|vip|link)/i.test( text );
		}
	}
