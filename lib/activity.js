...
class Activity {
	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+\.)?(3|jd|vip|tmall|taobao|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|tb|meituan|dpurl|didi|suvmothq|yuebai|yuebuy|maishou)\.(com|hk|cn|tv|plus|top|love|ren|link)/i.test( text );
		}
	}
	...
	*/
	static detectApp(text) {
		return /#小程序:\/\/(快手|拼多多)(特惠|福利券|限时折扣|短视频)\/(.+)/i.test( text ) || /mp:\/\/(.+)/i.test( text );
	}
	 ...
	 * @return {Array}
	 */
	static collect( caller, method, product, rawdata, extra = {}, end ){
		...
		if( typeof this.queue == 'undefined' ){
			this.queue = { 'date' : date };
		}
		*/

		//调用存储过程写入数据
		//CALL func_weixin_effect(`package`, `method`, `platform`, `item_id`, `extra`, `num`, `created_time`)

		let yymd = new Date(rawdata.created * 1000).format('yyyyMMdd');
		let hash = method + tail + yymd;
		...
		if( size > 500 || end ){
			...
			}, 1000 * ( end ? 30 : this.randomNum( 2, 10 ) ) );

		}

		//return { 'sql': sql, 'req': req, 'num': size, 'end': end };

		return { 'package': rawdata.package, 'product': product, 'num': size, 'end': end };
		...
		let hash = method + tail + yymd;
		*/
		let memo = caller.sider || caller.redis;

	    memo.get( hash, (err, ret) => {
			...
			if( ret ){
				...
				let ext = Object.assign( obj.ext, extra );

				let sql = 'UPDATE `pre_weixin_effect` SET sends_num = sends_num + 1, updated_time = ?, extend = ? WHERE auto_id = ?';
				let req = [ time, JSON.stringify( ext ), obj.id ];

				//更新群发记录，临时存储一天
				caller.mysql.query(sql, req, function( err, ret ){
					if( err ) return console.error( err );
					memo.set(hash, JSON.stringify( {id : obj.id, ext : ext} ) );
					memo.expire(hash, 3600 * 24);
				});

			}else{

				let ext = JSON.stringify( extra );

				let sql = 'REPLACE INTO `pre_weixin_effect` (package, method, platform, item_id, item_suffix, sends_num, extend, created_time, created_date) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)';
				let req = [ rawdata.package, method, product.platform, rwid, tail, 1, ext, rawdata.created, yymd ];

				//记住自增ID，临时存储一天
				caller.mysql.query(sql, req, function( err, ret ){
					if( err ) return console.error( err );
					memo.set(hash, JSON.stringify( {id : ret.insertId, ext : extra} ) );
					memo.expire(hash, 3600 * 24);
				});
	...
	static getExternal( text ) {
		...
		if ( text.indexOf('百亿') > -1 || text.indexOf('百补') > -1 || text.indexOf('百亿补贴') > -1 ) {
