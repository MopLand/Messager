...
class Activity {
	...
	*/
	static detectTbc(text, size) {
		...
		var mat = new RegExp('(\\W+)([a-zA-Z0-9]{'+ ( size || '10,14' ) +'})(\\W+)').exec( text + ' ' );
	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+\.)?(jd|vip|tmall|taobao|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|tb|meituan|dpurl|didi)\.(com|hk|cn)/i.test( text );
		}
	}
	...
	*/
	static detectApp(text) {
		return /#小程序:\/\/拼多多(特惠|福利券|限时折扣)\/(.+)/i.test( text ) || /mp:\/\/(.+)/i.test( text );
	}
	...
	static collect( caller, method, product, rawdata, extra = {}, end ){
		...
		*/
		...
		if( size > 500 || end ){
			...
			}, 1000 * ( end ? 45 : this.randomNum( 1, 15 ) ) );
