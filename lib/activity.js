...
class Activity {
	...
	*/
     ...
     * @param object 扩展信息
     */
	static collect( db, method, product, msgid = '', extra = {} ){
		...
		if( this.goods[ hash ] ){
			...
			let sql = 'UPDATE `pre_weixin_effect` SET sends_num = sends_num + 1, updated_time = ?, extend = ? WHERE method = ? AND platform = ? AND item_suffix = ? AND created_date = ? AND package = ?';
			let req = [ time, JSON.stringify( extra ), method, product.platform, tail, date, msgid ];

			db.query(sql, req);
			
		}else{

			this.goods[ hash ] = { product, extra };
			...
			let req = [ msgid, method, product.platform, rwid, tail, 1, JSON.stringify( extra ), time, date ];
