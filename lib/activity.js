...
class Activity {
	...
	*/
	static detectUrl(text, rule) {
		if( rule ){
			...
			return /(https?:\/\/)?(\w+\.)?(3|u\.f(\d)url|jd|vip|tmall|taobao|1688|yangkeduo|suning|pinduoduo|kaola|youpin|jinritemai|douyin|t|tb|mt|meituan|dpurl|didi|huaxz|suvmothq|yuebai|yuebuy|maishou|my22|lg11|lg22|lg33|lg44|mlj94|vv1i|m6z|xingfan|navo|tb3|4kma|5kma|pk(\d+)|pink(\d+)|ujdxx|jdcom|y\-03|lg(\d+)|kurl(\d+)|kzurl(\d+)|ctrip)\.(com|cc|co|hk|cn|tv|fun|info|top|love|plus|tv|fit|art|pub|red|ren|vip|uno|link|mba|me)/i.test( text );
		}
	}
	...
	*/
	static detectApp(text) {
		return /#小程序:\/\/(快手|拼多多|唯品会)(特卖|特惠|福利券|限时折扣|短视频|优惠商品推荐|丨品牌正品折扣)\/(.+)/i.test( text ) || /mp:\/\/(.+)/i.test( text );
	}
