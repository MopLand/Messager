...
class Common {
	...
	*/
	...
	static parseXml( data ) {
		...
		if( func ){
			return parser.parseString(data, func);
		}else{
		...
		*/
		...
		const parser = new XMLParser({
			...
			allowBooleanAttributes: true, // 是否允许布尔属性
			parseNodeValue: true, // 是否解析节点值
			parseAttributeValue: true, // 是否解析属性值
			attributeNamePrefix: '@', // 属性名前缀（默认：'@_'）
			trimValues: false, // 是否删除值周围的空格
			processEntities: false, // 是否处理 XML 实体
		});
