...
class Common {
	...
	*/
     ...
     */
	static locked( text ) {
		let file = process.cwd() + '/.gitlock';
		fs.writeFileSync( file, text + ' ' + this.getTime(), ( ret ) => { console.log( 'Git Locked' ); } );
	}
	...
	static unlock( ) {
		let file = process.cwd() + '/.gitlock';
		fs.existsSync( file ) && fs.unlink( file, ( ret ) => { console.log( 'Git Unlock' ); } );
	}
