...
class Weixin {
		...
		return new Promise(function (resolve, reject) {
			...
			Request.get(url, arg, (code, body) => {
				...
				if (code == 200 && body.Code == 0) {
					...
					reject(body.Message || body.message);
				}
	...
	post(api, arg) {
		...
		return new Promise(function (resolve, reject) {
			...
			Request.post(url, arg, (code, body) => {
     ...
     */
	GetQrCode(wxId, groupID) {
		var api = 'Group/GetQrCode/' + wxId + '/' + groupID;
		return this.post(api);
	}
	...
	ChangePassword(wxId, newPassword = '', ticket = '') {
		...
		var ret = { newPassword, ticket, wxId };
		if (newPassword == '') {
	...
	OneChangePassword(wxId, password = '', newPassword = '') {
		...
		var ret = { password, newPassword, wxId };
		if (password == '') {
			delete ret.password;
		}
		if (newPassword == '') {
