...
class Weixin {
			...
			Request[method](url, arg, ( code, body ) => {
				...
				if ( data = valid( code, body ) ) {
					...
					if( !err && body.baseResponse ){
						err = JSON.stringify( body.baseResponse.errMsg ) != '{}' ? body.baseResponse.errMsg : body.baseResponse.ret;
					}
					reject( err );
					log.error( method, { 'request' : api, 'querys' : arg, 'response' : body, 'server' : host, 'error' : err } );
				}
     ...
     */
	AutoAuth(userName) {
		var api = 'wechat/autoauth?' + Common.buildReq( { userName } );
		var ret = { };
		return this.post(api, ret);
	}
	 ...
	 * @param string tbc 淘宝口令修改
	 * @param string tbctext 淘口令随机文案
     */
	NewSendMsg(userName, toUserName, content, msgSource = '', type = 1, tbc = false, tbctext = [] ) {
		...
		for( let i = 0; i < toUserName.length; i++ ){
			...
			if (tbc) {
				content = Act.randomTbc(tmp, tbctext);
			}

			if (!content || content == '') {
				log.info('空消息', {userName, toUserName: toUserName[i], sourceContent: tmp, tbcContent: content, tbc});
				continue;
			}
	 ...
	 */
	SnsUploadPut(userName, url) {
		var api = 'wechat/mmsnsupload?' + Common.buildReq( { userName } );
		var ret = JSON.stringify({ url: url});
	 ...
	 * 上传图片到微信 post
	 * @param string userName 微信ID
	 * @param string base64 图片base64格式
	 */
	SnsUpload(userName, base64) {
		var api = 'wechat/mmsnsupload?' + Common.buildReq( { userName } );
		var ret = {base64: base64};
	 ...
	 * 发送图片消息到微信 put
	 * @param string userName 微信ID
	 * @param string toUserName 发送微信ID或群ID
	 * @param string url 图片链接
	 */
	SnsUploadMsgPut(userName, toUserName, url) {
		var api = 'wechat/uploadimgmsg?' + Common.buildReq({ userName });
	 ...
	 * @param string base64 图片base64格式
	 */
	SnsUploadMsg(userName, toUserName, base64) {
		var api = 'wechat/uploadimgmsg?' + Common.buildReq({ userName });
		var ret = {
			toUserName: toUserName,
			base64: base64
