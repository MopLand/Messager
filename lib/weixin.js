...
class Weixin {
	...
	UploadMsgImg(userName, toUserName, totalLen, cdnbigImgUrl, cdnmidImgUrl, aeskey, encryVer, cdnbigImgSize, cdnmidImgSize, cdnthumbImgUrl, cdnthumbImgSize, cdnthumbImgHeight, cdnthumbImgWidth ) {
		var api = 'message/uploadmsgimg?' + Common.buildReq( { userName } );
	...
	SnsUploadPut(userName, url) {
		var api = 'sns/mmsnsupload?' + Common.buildReq( { userName } );
		var ret = { url: url };
	...
	SnsUploadMsgPut(userName, toUserName, url) {
		var api = 'sns/uploadimgmsg?' + Common.buildReq({ userName });
		var ret = {
			toUserName: toUserName,
			url: url
		};
