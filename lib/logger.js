	...
	clean( days = 7, subs = '/logs/', suffix = '.log' ){
		...
		file.filter( v => v.indexOf( suffix ) > 0 ).forEach( ele => {
			...
			fs.stat( node, ( err, info ) => {
				...
				//文件大于多少，清空内容，每日日志文件不主动清空
				if( info.size > size && /[0-9]{8}/.test(ele) == false ){
					fs.truncateSync( node );
					console.log( node, info.size, 'CLEAR' );
				}

				//更新于几天前，强制删除
				if( time - info.mtimeMs >= diff ){
					try {
						fs.unlinkSync( node );
						console.log( node, info, 'DELETE' );
					} catch ( err ) {
						console.error(err);
					}
				}
